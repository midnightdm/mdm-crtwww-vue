<template>
 <LogsSubMenu></LogsSubMenu>
  <div id="main" class="past24">
    <h1>{{this.$store.getters.getPast24.length}} Vessel Passage<span v-if='this.$store.getters.getPast24.length != 1'>s</span> in the Past 24 Hours</h1>
  <h4>{{ this.formattedRange }}</h4>
  <button v-bind:class="{ active: scrolled}" id="topbtn" @click="topOfPage">Top</button>
  <main>
    <ul class="vessels-list" v-for="vessel in this.$store.getters.getPast24" :key="vessel.vesselID">
    <li>
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit('setLogsLinkActive', true)
     this.$store.commit('setPageSelected', 'Past24')
  },
  unmounted() {
  ...
  computed: {
    formattedRange() {
      return "Range is "+
      format(this.$store.state.a.ranges.past24.lo*1000, "h:mm aaa eeee, LLL do") +
      " to " +
      format(this.$store.state.a.ranges.past24.hi*1000, "h:mm aaa eeee, LLL do")
